...
\end{itemize}
While the work in Chapter~\ref{C:ts} deals with the first category of optimism-based algorithms, the work described in the following deals with the second one. This is only partially true since we do not consider optimistic initialization, but we want the action-value to be an optimistic estimate which is propagated through the updates by the \gls{be}.
...
than Bayesian approaches, they have been able to obtain impressive results~\cite{bellemare2016unifying}. As already discussed in Chapter~\ref{C:ts}, \gls{im}
