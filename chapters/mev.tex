...
The estimation of the \glsfirst{mev} of a set of random variables is required in several applications. Indeed, almost any decision-making process of acting involves the optimization of an expected utility function. For example, in everyday life decisions are usually made by considering the possible outcomes of each action based on partial information.
While sometimes only the order of preference of these alternatives matters, many applications require an explicit estimation of the maximum utility.
For instance, in \gls{rl} the optimal policy can be found by taking, in each state, the action that attains the maximum expected cumulative reward. The optimal value of an action in a state, in turn, depends on the \glspl{mev} of the actions available in the reached states.
Since the errors propagate through all state-action pairs, a bad \gls{mev} estimator negatively affects learning speed~\cite{van2010double}.

The most used approach to this estimation problem is the \gls{me} which simply takes the maximum estimated utility. As proved in~\cite{smith2006optimizer}, this estimate is positively biased and, if used in iterative algorithms, can increase the approximation error step-by-step~\cite{van2010double}. More effective estimators have been proposed in recent years. The \gls{de}~\cite{van2013estimating} approximates the maximum by splitting the sample set into two disjoint sample sets. One of these sets is used to pick the element with the maximum approximate value and its value is picked from the other set. This must also be done in the opposite way by switching the role of the two sets. Eventually, the average (or a convex combination) of the two values is considered. This approach has been proven to have a negative bias~\cite{van2013estimating} which, in some applications, allows to overcome the problem of \gls{me}.

During my years of Ph.D. research, we analyzed this problem and proposed the \gls{we}~\cite{deramo2016estimating} which approximates the \gls{mev} of a set of random variables by a sum of different values weighted by their probability of being the maximum. \gls{we} can have both negative and positive bias, but its bias always stays in the range between the \gls{me} and \gls{de} biases.

All the mentioned approaches are limited to a finite set of random variables, thus, in a subsequent work, we extended the study to problems with an infinite set of random variables and proposed an extension of \gls{we}~\cite{deramo2017maximum} to address them.
...
\end{align}
Unfortunately, $\mu_{*}(X)$ cannot be found analytically when the \glspl{pdf} are unknown.
...
\subsection{Related works}\label{S:mev_works}
Several methods for estimating the \gls{mev} have been proposed in the literature. The most straightforward one is the \gls{me} which consists in approximating the \gls{mev} with the maximum of the sample means:
\begin{equation}\label{E:biasME}
\est{ME}(S) = \max_{i}\hat{\mu}_{i}(S) \approx \mu_{*}(X).
\end{equation}
Unfortunately, as proved in~\cite{smith2006optimizer}, this estimator has a positive bias that may cause issues in applications of \gls{me}. For instance in the \gls{ql} algorithm the overestimation of the state-action values due to the positive bias can cause an error that increases step after step. However, the expected value of the \gls{me} is different from the \gls{mev} in~(\ref{E:maxExp}). Consider the \gls{cdf} $\hat{F}_{\max}(x)$ of the \gls{me} $\max_{i}\hat\mu_{i}$ corresponding to the probability that \gls{me} is less than or equal to $x$. This probability is equal to the probability that all other estimates are less than or equal to $x$: 
...
To solve this overestimation problem, a method called \gls{de} has been proposed in~\cite{van2010double} and theoretically analyzed in~\cite{van2013estimating}. \gls{de} uses a sample set $S$ retrieved from the true unknown distribution like \gls{me}, but splits it into two disjoint subsets $S^A = \lbrace S^A_{1}, ..., S^A_{N} \rbrace$ and $S^B = \lbrace S^B_{1}, ..., S^B_{N} \rbrace$. If the sets are split in a proper way, for instance randomly, the sample means $\hat{\mu}^A_{i}$ and $\hat{\mu}^B_{i}$ are unbiased, like the means $\hat{\mu}_{i}$ in the case of the \gls{me}. An estimator $a^*$, such that $\hat\mu^A_{a^*}(X) = \max_{i}\hat\mu^A_{i}(X)$, is used to pick an estimator $\hat\mu^B_{a^*}$ that is an estimate for $\max_{i}E [ \hat\mu^B_{i} ]$ and for $\max_{i}E [ X_{i} ]$. Obviously, this can be done in the opposite way, using an estimator $b^*$ to retrieve the estimator value $\hat{\mu}^A_{b^*}$. 
...
This formula can be seen as a weighted sum of the expected values of the random variables where the weights are the probabilities of each variable being the maximum. Since these probabilities sum to one, the approximation given by \gls{de} results in a value less than or equal to the \gls{mev}. Even if the underestimation does not guarantee a better estimate than the \gls{me}, it may be helpful to avoid an incremental approximation error in some learning problems. For instance, \gls{dql}~\cite{van2010double} is a variation of \gls{ql} that exploits this technique to avoid the previously described issues due to overestimation. \gls{dql} has been tested in some very noisy environments and succeeded to find better policies than \gls{ql}. Another remarkable application of \gls{de} is presented in~\cite{xu2013mab} where it achieves better results than \gls{me} in a sponsored search auction problem.

\section{Weighted Estimator}
Differently from \gls{me} and \gls{de} that output the sample mean of the variable that is estimated to be the one with the largest expected value, the proposed \gls{we} estimates the \gls{mev} $\mu_*(X)$ computing a weighted average of all the sample means:
\begin{equation}\label{E:WE}
\est{WE}(S) = \sum_{i=1}^M \hat\mu_i(S) w_i^S.
\end{equation}
Ideally, each weight $w_i^S$ should be the probability of $\hat\mu_i(S)$ being larger than all other sample means:  
...
Unfortunately, its \gls{pdf} $\hat f_i^S$ depends on the \gls{pdf} $f_i$ of variable $X_i$ which is assumed to be unknown.
In particular, if $X_i$ is normally distributed, then, regardless of the sample size, the sampling distribution of its sample mean is normal too: $\hat\mu_i(S)\sim\mathcal{N}\left(\mu_i,\frac{\sigma_i^2}{|S_i|}\right)$.
On the other hand, from the \gls{clt}, the sampling distribution $\hat f_i^S$ of the sample mean $\hat\mu_i(S)$ approaches the normal distribution as the number of samples $|S_i|$ increases, regardless of the distribution of $X_i$.
Leveraging these considerations, we propose to approximate the distribution of the sample mean $\hat\mu_i(S)$ with a normal distribution, where we replace the (unknown) population mean and variance of variable $X_i$ with their (unbiased) sample estimates $\hat\mu_i(S)$ and $\hat\sigma_i(S)$:
...
It is worth noting that \gls{we} is consistent with $\mu_*(X)$. In fact, as the number of samples grows to infinity, each sample mean $\hat\mu_i$ converges to the related population mean $\mu_i$, and the variance of the normal distribution $\tilde f_i$ tends to zero, so that the weights of the variables with an expected value less than $\mu_*(X)$ go to zero, so that $\est{WE} \rightarrow \mu_*(X)$.

\subsection{Generalization to infinite random variables}\label{S:infinite}
As far as we know, the previous literature has focused only on the finite case and no approaches that natively handle continuous sets of random variables (e.g. without discretization) are available.
Let us consider a continuous space of random variables $\mathcal{Z}$ equipped with some metric (e.g. a Polish space) and assume that the variables in $\mathcal{Z}$ have some spatial correlation.
Here, we consider $\mathcal{Z}$ to be a closed interval in $\mathbb{R}$ and that every variable $z \in \mathcal{Z}$ has unknown mean $\mu_z$ and variance $\sigma^2_z$.
Given a set of samples $S$, we assume to have an estimate $\hat{\mu}_z(S)$ of the expected value $\mu_z$ for any variable $z \in \mathcal{Z}$ (in the next section we will discuss the spatial assumption and explain how to obtain this estimate).
As a result, the weighted sum of Equation~(\ref{E:WE}) generalizes to an integral over space $\mathcal{Z}$:
\begin{align}\label{E:continuousWE}
\hat{\mu}_*^{\textrm{WE}}(S) = \int_{\mathcal{Z}} \hat{\mu}_z(S) \, 
\mathfrak{f}_z^*(S) \mathrm{d}z ,
\end{align}
where $\mathfrak{f}_z^*(S)$ is the probability density for $z$ of \emph{being the variable with the largest mean}, which plays the same role of the weights used in~(\ref{E:WE}).
...
\end{enumerate}
In order to face the former issue, we assumed the random variables to be spatially correlated.
...
To address the second issue, we need to restrict the regression class to the methods for which it is possible to evaluate the uncertainty of the outcome.
...
While several regression techniques can be exploited (e.g. linear regression), the natural choice, in this case, is the \gls{gp} regression since it provides both an estimate of the mean and variance of the process.
Having a \gls{gp} trained on a dataset of $N$ samples $\mathcal{D}=\{z_i, q_i\}_{i=1}^{N}$, where $q_i$ is a sample drawn from the distribution of $z_i$.
...
If we look at absolute bias, there is no clear winner. 
For instance, when all random variables are distributed identically, \gls{de} is unbiased, while the same setting represents the worst case for \gls{me}.
On the other hand, when the \gls{mev} is sufficiently larger than the expected values of the other variables, the absolute bias of \gls{me} may be significantly smaller than the one of \gls{de}.
...
\end{theorem}
\begin{proof}
The proof of this Theorem follows directly from observing that $\est{WE}$ is always smaller than $\est{ME}$. In fact, the \gls{me} estimator can be seen as a weighted estimator that gives probability one to the variable associated to the largest sample mean $\hat\mu_i$, so that any other weighting cannot produce a larger value. 
\end{proof}
...
\end{theorem}
\begin{proof}
  If we compare the expected value of \gls{de} reported in Equation~\eqref{E:biasCV} with the value of the  estimator \gls{we} in Equation~\eqref{E:WE}, we can notice strong similarities.
  The main difference is that in \gls{de} the sample mean of variable $X_i$ and its probability of being the maximum are computed w.r.t. two independent set of samples, while in \gls{we} these two quantities are positively correlated. It follows that \gls{we} has a positive bias w.r.t. \gls{de}.
\end{proof}

\textbf{Example} In Figures~\ref{F:bias} and~\ref{F:absolute_bias} we visualize the bias of the different \gls{mev} estimators in a setting with two normally distributed random variables ($X_1\sim\mathcal{N}(\mu_1,\sigma_1^2)$ and $X_2\sim\mathcal{N}(\mu_2,\sigma_2^2)$) as a function of the difference in their expected values. Both variables have a variance equal to $10$ ($\sigma_1^2=\sigma_2^2=10$) and we assume to have $100$ samples for each variable ($|S_1|=|S_2|=100$).
...
Looking at the absolute biases shown in Figure~\ref{F:absolute_bias}, we can notice that there is not clear winner.
As previously mentioned, when the variables have the same mean, both \gls{de} and \gls{dwe} are unbiased, while it represents the worst case for the bias of \gls{me} and \gls{we}. It follows that, when the difference between the two means is small (less than $0.5$ in the example), \gls{de} suffers less absolute bias than \gls{me} and \gls{we}. For moderate differences of the means (between $0.5$ and $1.8$ in the example), \gls{we} has the lowest absolute bias, while \gls{me} is preferable for larger differences.
Such results can be generalized as follows: \gls{de} suffers a small bias when there are several random variables that have expected values close (w.r.t.~their variances) to the maximum one, while \gls{me} provides the best estimate when there is one random variable whose expected value is significantly larger (w.r.t.~the variances) than all expected values of all other variables. In all other cases, \gls{we} is less biased.
...
$ \mathrm{Var}\left(\est{ME}\right) \leq \sum_{i=1}^M \frac{\sigma^2_i}{|S_i|}$ and $\mathrm{Var}\left(\est{DE}\right) \leq \sum_{i=1}^M \frac{\sigma^2_i}{|S_i|}$.
...
\end{theorem}
\begin{proof}
 Starting from the definition of \gls{we}~\ref{E:WE}, we can derive the bound to the variance as follows
 \begin{align*}
...
\end{theorem}
\begin{proof}
 Since the weights $w_i$ computed by \gls{dwe} are not random variables, it follows
 \begin{align*}
  \mathrm{Var}\left(\est{DWE}\right) &= \mathrm{Var}\left(\sum_{i=1}^M \hat\mu_i(S) w_i\right)\\
...
Among value-based methods, we consider online and offline algorithms that approximate the optimal action-value function $Q^*$ without the need for a model of the environment. We mostly consider \glspl{mdp} with discrete action spaces, except for the batch algorithm based on \gls{we} that can also be extended to \glspl{mdp} with continuous action spaces.

\subsection{Online}
It is demonstrated that since \gls{ql} (see Section~\ref{S:Q-Learning}) is a stochastic approximation algorithm, under the assumption that every state-action pair is visited infinitely often and the step sequence satisfies certain conditions, $Q_k$ converges to $Q^{*}$~\cite{watkins1989learning}. However, under particular conditions, such as wrong tuning of parameters and noisy environments, the $Q$-function may converge to $Q^{*}$ too slowly. One of the main reasons is that the \gls{ql} uses the \gls{me} to estimate the current maximum $Q$-value of the next state $s'$. Since this estimator is positively biased, and since the error is propagated at each step, the $Q$-function can be wrongly estimated and the algorithm may fail.
...
\subsubsection{Weighted $Q$-Learning}\label{S:wql}
We propose to replace \gls{me} with \gls{we} in the \gls{wql} algorithm~\cite{deramo2016estimating}. \gls{wql} maintains an estimate of the mean value of the $Q$-function and its variance to compute the weights of \gls{we} (Algorithm \ref{A:WQ-Learning}).
While the mean value corresponds to the current estimate of the $Q$-function, the variance is not straightforward to be computed. Indeed, it is not simply the variance of the $Q$-function approximator, but it is the variance of the process consisting of an update formula with a variable learning rate. Considering this, it can be shown that the variance can be computed incrementally at each step $t$ with:
...
\subsection{Batch}
The \gls{fqi} update (see Section~\ref{S:FQI}), similarly to the \gls{ql} update, requires the computation of \gls{me} which causes the same overestimation problem of \gls{ql}. Intuitively, the replacement of \gls{me} with \gls{de} or \gls{we} can help to solve this issue also in \gls{fqi}.
...
The \gls{fqi} variant that replaces \gls{me} with \gls{de} is called \gls{dfqi}, and the variant that we propose using \gls{we} is called \gls{wfqi}~\cite{deramo2017maximum}. \gls{wfqi} uses \gls{gp} regression to compute the mean $Q$-value and its variance in continuous state spaces (Algorithm \ref{A:WFQI}). The interesting aspect of \gls{wfqi} is that it can handle also infinite action spaces, as explained in Section \ref{S:infinite} and shown in Algorithm \ref{A:continuousWFQI}. To the best of our knowledge, this makes it the only value-based algorithm able to deal with infinite action spaces.

\subsection{Deep Reinforcement Learning}
We also analyzed the case of \gls{drl} where the overestimation problem caused by \gls{me} occurs also in the \gls{dqn} algorithm resulting in critical loss of performance in some problems due to instability. The \gls{ddqn} algorithm~\cite{hasselt2015double} replaces \gls{me} with \gls{de} and shows considerable improvements in performance and stability. We worked on the replacement of \gls{me} with \gls{we} even though in this case it is not straightforward due to the difficulties in computing the variance of the approximation. The \gls{gp} regression, used in \gls{wfqi}, is not commonly used in \gls{drl} and a deep neural network adapts better in important \gls{drl} problems, such as the well-known Atari domain~\cite{bellemare2013arcade}. The estimation of the mean and variance with a neural network is possible, but the computational complexity increases with the number of parameters such that makes it unfeasible in deep neural networks.
...
\end{figure}
We propose to estimate the variance of the approximation using an ensemble of target networks, following the neural network architecture proposed in another algorithm called \gls{bdqn}~\cite{osband2017deep} shown in Figure~\ref{F:bdqn-net}. This work follows the \gls{dqn} algorithm described in Section~\ref{S:dqn} with few, but important changes. The output of the neural network is split into $K$ heads that share the same first hidden layers. To perform bootstrap, a binary mask $w_1, \dots, w_K \in \lbrace 0,1 \rbrace$ is assigned to each sample to indicate the heads assigned to it. The binary mask is generated with a binomial distribution with probability $p$. The exploration policy of \gls{bdqn} uniformly samples a head at the beginning of the episode, and follows the greedy policy learned by it. During the learning phase, the different heads are updated following the update rule of \gls{ddqn} to avoid the overestimation problem.

We propose to use the architecture of \gls{bdqn} and use \gls{we} to compute the target of the $K$-th head, while using the target provided by \gls{me} for the other $K-1$ heads. Moreover, we use only the $K$-th head trained with \gls{we} for exploration. In this way, the training of the $K-1$ heads does not differ from the one of \gls{bdqn}, while the $K$-th head is trained with \gls{we} exploiting the uncertainty on the action values provided by the ensemble. Given a dataset of transitions $\langle s_t, a_t, r_t, s'_t \rangle$, the resulting formulas to compute the target $y^k$ of each head $k$ are:
...
We evaluate the performance of \gls{me}, \gls{de} and \gls{we} in \gls{mab} and \gls{rl} problems. We start by considering discrete state and action spaces, then we move onto the continuous ones and, eventually, to \gls{drl} problems.
...
In the last setting, we modify the interval of the mean \gls{ctr} by changing the value of the upper limit with values in $\lbrace 0.02, 0.03, ..., 0.09, 0.1 \rbrace$, with the lower limit fixed at $0.02$.
...
In the first setting (Figure~\ref{F:ia_first}) the \gls{mse} reasonably decreases for all the estimators as the number of impressions increases and WE has the lowest \gls{mse} in all cases. Interestingly, the \gls{me} estimator has a very large bias in the leftmost case, showing that the \gls{me} estimator suffers large bias when the variances of the sample means are large due to the lack of samples (as shown also in Figure \ref{F:absolute_bias}).
Figure~\ref{F:ia_second} shows that an increasing number of actions has a negative effect on \gls{me} and a positive effect on \gls{de} due to the fact that a larger number of ads implies a larger number of variables with a mean close to the \gls{mev} that represents a worst case for \gls{me} and a best case for \gls{de}.
The \gls{mse} of \gls{we} is the lowest in all cases and does not seem to suffer the increasing number of actions. 
The same happens in Figure~\ref{F:ia_third} when all the ads share the same \gls{ctr} ($0.02$), where \gls{de} is the best.
However, it starts to have a large variance as soon as the range of probabilities increases (Figure \ref{F:variance}). 
...
\glspl{ctr} are generally unknown, therefore the search engine should use the data to estimate which is the best ad (i.e., the one that maximizes $b\cdot\rho$) and the payment in case of a click; reasonably, wrong estimates may significantly harm the revenue.
On the other hand, advertisers have to decide the value of their bid $b_i$ according to the true values $v_i$ of a click. A desirable condition in auctions, called \textit{incentive compatibility}, requires advertisers to maximize their utility by truthfully bidding $b_i = v_i$. Incentive compatibility may not occur when the estimates of the click probabilities are not accurate. We want to evaluate how the estimators favor the incentive compatibility.
We measure the utility gain of advertiser $1$, whose true value per click is $v_1 = 1$, for different bid $b_1$ values and competing with four other advertisers whose bids are $b_{-1} = \lbrace 0.9, 1, 2, 1 \rbrace$. The \glspl{ctr} are: $\rho = \lbrace 0.15, 0.11, 0.1, 0.05, 0.01 \rbrace$. 
\glspl{ctr} are estimated from data collected using the UCB1 algorithm~\cite{auer2002finite} in a learning phase consisting of $10,000$ rounds of exploration (i.e. impressions), as done in~\cite{xu2013mab}.
...
In this domain, we compare the three estimators together with the performance of an algorithm called Bias-Corrected \gls{ql}, a modified version of \gls{ql} which, assuming Gaussian rewards, corrects the positive bias of \gls{me} by subtracting to each $Q$-value a quantity that depends on the standard deviation of the reward and on the number of actions~\cite{lee2012intelligent,lee2013bias}. Moreover, we also test \gls{wql} using a different policy, that we call \textit{weighted policy}, which samples the action to be performed in a state from the probability distribution of the weights of WE.
...
The learning rate is $\alpha_t(s, a) = \frac{1}{n_t(s, a)^{0.8}}$ where $n_t(s, a)$ is the current number of updates of that action value and the discount factor is $\gamma = 0.95$. 
...
Once in the goal state, each action ends the episode and returns a reward of $5$. The optimal policy ends the episode in five actions, therefore the optimal average reward per step is $0.2$. Moreover, the optimal value of the action maximizes the $Q$-value is $5\gamma^4 - \sum_{k=0}^3 \gamma^k \approx 0.36$.
...
\begin{figure*}
  ...
  \end{minipage}
\end{figure*}
Figures~\ref{F:bernoulli} and~\ref{F:gaussian5} show that, regardless of the bad approximation of the $Q$-function, the underestimation of \gls{dql} allows to learn the best policy faster than other algorithms in these noisy settings.
...
\gls{wql} shows much less bias than the other estimators in all settings; moreover, the use of the weighted policy generally reduces the bias of the estimation and achieves the best performance in the case with $\sigma = 1$ (see Figure~\ref{F:gaussian1}). These good results are explained considering that the weighted policy is able to reduce exploration faster than $\varepsilon$ greedy due to the exploiting of the good approximation of the $Q$-function computed by \gls{wql}. It is worth noting that \gls{wql} works well for both Gaussian and Bernoullian rewards, showing that \gls{we} is also effective with non-Gaussian distributions even if it uses a Gaussian approximation of $Q$-values,

\begin{figure*}
\begin{minipage}{\textwidth}
\centering
\subfigure[Training phase.\label{F:forex_train}]{\includegraphics[scale=0.6]{./img/forexTrain.pdf}}
\subfigure[Test phase.\label{F:forex_test}]{\includegraphics[scale=0.6]{./img/forexTest.pdf}}
\end{minipage}
...
We build an \gls{mdp} based on the Foreign Exchange Market (Forex), an environment with acknowledged hardly predictable dynamics that complicate the estimation of the $Q$-values and, therefore, of the expected profit. The \gls{mdp} we build is a simplified version of the real Forex market. In our Forex \gls{mdp}, the agent always enters in the market with $1$\$ and each time the agent enters on long or short position a fixed spread value of $0.0002$\$ is paid.
The possible actions taken by the agent can be $-1$, $0$ or $1$, which mean respectively \textit{'enter on a short position'}, \textit{'close a position'} and \textit{'enter on long position'}.
...
The same algorithms used in the grid world domain, except for Bias-Corrected \gls{ql}, were trained using the daily historical data of GBP/USD exchange rate from 09/22/1997 to 01/10/2005 and tested on data from 01/11/2005 to 05/27/08. 
During the training phase, we set the learning rate $\alpha(s,a)=\frac{1}{n(s, a)}$, discount factor $\gamma=0.8$ and $\varepsilon=\frac{1}{\sqrt{n(s)}}$.

Figure \ref{F:forex} shows the annual profit, w.r.t. the number of training episodes, of the four algorithms during the training phase (Figure~\ref{F:forex_train}) and during a test phase where the learned policy is run with a greedy policy (Figure~\ref{F:forex_test}). In the training phase, an $\varepsilon$-greedy policy is used for \gls{ql} and \gls{dql}, while \gls{wql} uses both the $\varepsilon$-greedy policy and the weighted policy.
During the training phase, \gls{ql} performs better than \gls{dql} and also than \gls{wql}. Interestingly, \gls{wql} with the weighted policy reaches the worst performance in the training phase, but reaches the best performance in the test phase. This happens because more exploration is induced by the weighted policy w.r.t. the $\varepsilon$-greedy policy, resulting in bad performance during the training phase, but also in better estimates of the $Q$-values.
\gls{dql} performs worse than \gls{ql} and \gls{wql} in both training and testing phase. 
The reason is that in many states there is a significantly better action than the others, which represents the case where \gls{me} gives the best results, and the case where \gls{de} suffers the most.
...
The support (action) space is bounded but continuous, and represents the price $p$ to show to the user ($p \in [0,10]$).
...
\gls{we} exploits a Gaussian process with a squared exponential kernel to generalize over the continuous price (\gls{gp} parameters are learned from $\mathcal{D}$), while \gls{me} and \gls{de} discretize the price space into $n_b$ evenly spaced bins.
As shown in Figure~\ref{F:pricing_bias}, the number $n_b$ of optimal bins varies with the number $n_s$ of available samples.
This means that, once the samples have been collected, \gls{me} and \gls{de} need an optimization phase to select the appropriate number of bins (not required by \gls{we}).
...
This is not the case for pricing because this problem is not iterated. From Figure~\ref{F:pricing_bias} we can see that \gls{me} is prone to provide positive bias, while \gls{we} bias is almost always the smallest or stays between \gls{me} and \gls{de}.
The reason why the \gls{me} bias is not always positive, as stated by its theoretical property (for finite case), is due 
...
\begin{minipage}{\textwidth}
\centering
\subfigure[Discrete actions.\label{F:pendulum_discrete}]{\includegraphics[scale=0.35]{./img/pendulumDiscrete.pdf}}
\hspace{-.5cm}
\subfigure[Continuous actions.\label{F:pendulum_continuous}]{\includegraphics[scale=0.35]{./img/pendulumContinuous.pdf}}
\end{minipage}
...
The \gls{gp} uses a squared exponential kernel with an independent length scale for each input dimension (ARD SE). The hyperparameters are fitted on the samples and the input values are normalized between $[-1,1]$.
...
Figure~\ref{F:pendulum} shows that \gls{wfqi} for discrete actions \ref{A:WFQI} and \gls{fqi} are robust with respect to the number of episodes and \gls{wfqi} reaches the highest average reward in each case (with statistical confidence obtained over $100$ runs and $95\%$ level).
\gls{dfqi} performance is reasonably poor with a few examples since it uses half of the training set to train each regressor.
In the latter setting, the only algorithm able to directly handle the continuous space is the \gls{wfqi} defined in Algorithm~\ref{A:continuousWFQI}.
